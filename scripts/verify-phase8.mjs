...
if (!['8.1', '8.2', '8.3', '8.4'].includes(phase)) throw new Error('Expected Phase 8 step');
const out = `.local/verification/phase8/${phase}`;
...
const scripts = ['check:types', 'check:lint', 'build:keychain', 'build', 'test:unit', 'test:integration', 'test:evals', 'verify:planning', 'test:workflow', 'verify:step'];
const results = scripts.map((script) => {
  const run = spawnSync(runtimeNpm, ['run', script], { env: { ...process.env, PATH: `${runtimeBin}${path.delimiter}${process.env.PATH ?? ''}`, TMPDIR: path.resolve('.local/phase8/tmp'), npm_config_cache: path.resolve('.local/phase8/npm-cache'), XDG_CACHE_HOME: path.resolve('.local/phase8/cache') }, encoding: 'utf8' });
...
const report = { phase, time: new Date().toISOString(), sourceHashes: before, sourceUnchanged, results,
  ownerOnlyPending: ['desktop rendered design review', 'UI/UX manual tests', 'browser/accessibility checks', 'Phase 8 owner acceptance', 'User-owned in-app authentication and live use are deferred; developer credential tests are excluded by CHANGE-P8-BYO'],
