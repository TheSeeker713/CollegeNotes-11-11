...
if (!['9.1', '9.2', '9.3', '9.4'].includes(phase)) throw new Error('Expected Phase 9 step');
const out = `.local/verification/phase9/${phase}`;
...
const results = scripts.map((script) => {
  const run = spawnSync(runtimeNpm, ['run', script], { env: { ...process.env, PATH: `${runtimeBin}${path.delimiter}${process.env.PATH ?? ''}`, TMPDIR: path.resolve('.local/phase9/tmp'), npm_config_cache: path.resolve('.local/phase9/npm-cache'), XDG_CACHE_HOME: path.resolve('.local/phase9/cache') }, encoding: 'utf8' });
...
const report = { phase, time: new Date().toISOString(), sourceHashes: before, sourceUnchanged, results,
  ownerOnlyPending: ['desktop rendered design review', 'UI/UX manual tests', 'browser/accessibility checks', 'Phase 9 owner acceptance', 'User-owned in-app authentication and live use are deferred; developer credential tests are excluded by CHANGE-P8-BYO'],
