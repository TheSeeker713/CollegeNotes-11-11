...
if (!['7.1', '7.2', '7.3', '7.4'].includes(phase)) throw new Error('Expected Phase 7 step');
const out = `.local/verification/phase7/${phase}`;
...
const results = scripts.map((script) => {
  const run = spawnSync(runtimeNpm, ['run', script], { env: { ...process.env, PATH: `${runtimeBin}${path.delimiter}${process.env.PATH ?? ''}`, TMPDIR: path.resolve('.local/phase7/tmp'), npm_config_cache: path.resolve('.local/phase7/npm-cache'), XDG_CACHE_HOME: path.resolve('.local/phase7/cache') }, encoding: 'utf8' });
...
const report = { phase, time: new Date().toISOString(), sourceHashes: before, sourceUnchanged, results,
  ownerOnlyPending: ['desktop rendered design review', 'UI/UX manual tests', 'browser/accessibility checks', 'Phase 7 owner acceptance'],
