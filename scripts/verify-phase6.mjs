...
if (!['6.1', '6.2', '6.3', '6.4'].includes(phase)) throw new Error('Expected Phase 6 step');
const out = `.local/verification/phase6/${phase}`;
...
const results = scripts.map((script) => {
  const run = spawnSync(runtimeNpm, ['run', script], { env: { ...process.env, PATH: `${runtimeBin}${path.delimiter}${process.env.PATH ?? ''}`, TMPDIR: path.resolve('.local/phase6/tmp'), npm_config_cache: path.resolve('.local/phase6/npm-cache'), XDG_CACHE_HOME: path.resolve('.local/phase6/cache') }, encoding: 'utf8' });
...
const report = { phase, time: new Date().toISOString(), sourceHashes: before, sourceUnchanged, results,
  ownerOnlyPending: ['desktop rendered design review', 'UI/UX manual tests', 'browser/accessibility checks', 'Phase 6 owner acceptance'],
