...
export function exportMaterials(store:Store,courseId:string,ids:unknown) {
  ...
  return store.db.transaction(()=>{
    ...
    const sources=materials.map(m=>{return {...publicMaterial(m),originalBase64:checkedOriginal(store,m).bytes!.toString('base64'),revisions:materialDetail(store,courseId,m.id).revisions,annotations:store.db.prepare('select * from reading_annotations where source_id=? and course_id=?').all(m.id,courseId),readingPositions:store.db.prepare('select * from reading_positions where source_id=? and course_id=?').all(m.id,courseId)};});
