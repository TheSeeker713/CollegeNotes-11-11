...
import {readingAnnotations,addAnnotation,changeAnnotation,readingPosition,saveReadingPosition,readingDocument,requireMaterial,readOriginal,checksum,CourseError,type Store} from '@collegenotes/storage';
...
export function readingRoutes(app:FastifyInstance,store:Store){
 ...
 app.get('/courses/:courseId/reading/:sourceId/position',async request=>{const p=ids(request.params);return readingPosition(store,p.courseId,p.sourceId);});
