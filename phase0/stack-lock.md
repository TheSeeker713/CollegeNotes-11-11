...
React + TypeScript + Vite provide the main interface; semantic HTML/CSS/SVG handle ordinary content. React Three Fiber 9 uses React 19; Three.js provides selected WebGL2 objects. Chrome Stable is the supported interim local development shell, with WebGL2 capability detection and a graphics-disabled equivalent; final Phase 14 acceptance packages the same local app experience for macOS so the user does not run a development server. This project does not target hosted web, phone, tablet or cross-platform clients. WebGPU is deferred. Existing browser version and hardware details stay in private environment evidence; no actual rendering or performance pass is claimed. [R3F documentation](https://r3f.docs.pmnd.rs/getting-started/introduction), [Vite requirements](https://vite.dev/guide/).

The exact direct dependency list follows; installation-manifest.json is the machine-readable decision, not an installed dependency lockfile. Published metadata is archived privately with engines and peer ranges. 39 applicable Node/peer constraints were checked with the existing npm semver library; all passed. Optional peers for unselected preprocessors, native mobile, compiler extras and alternate test environments are explicitly excluded, not skipped required tests. TypeScript 7.0.2 was rejected because typescript-eslint requires <6.1.0; select 6.0.3. Three.js 0.185.1 matches the 0.185 type family rather than using 0.186 with older declarations. These were the latest mutually compatible stable selections at the Phase 0 lock date, not a permanent claim that they remain newest. At the start of each dependency-bearing implementation phase, recheck current stable releases, security advisories, Node/peer compatibility and migration notes using primary publisher sources; upgrade only as an explicit, fully tested lock amendment.
...
The required local embedding runtime/model and final macOS wrapper are deliberately not invented in this Phase 0 package list. Their selection gates are Step 6.4 and Step 14.1 respectively: each must be current, Apple-silicon compatible, licensed, pinned, measured on REF-MAC-01 and integrated without changing the browser-era domain contracts. OAuth research should prefer standards-based server-side/local-companion flows and macOS credential storage rather than adding tokens to the React client.
